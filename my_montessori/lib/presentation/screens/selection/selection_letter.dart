...
class SelectionLetterScreen extends StatefulWidget {
  final int index;
  final bool initialIsUppercase;

  const SelectionLetterScreen({
    Key? key,
    required this.index,
  ...
  @override
  State<SelectionLetterScreen> createState() => _SelectionLetterScreenState();
}

class _SelectionLetterScreenState extends State<SelectionLetterScreen> {
  final _random = Random();
  late final int _entryIndex;
  late final Letter _currentLetter;
  ...
  void initState() {
    super.initState();
    _entryIndex = letters.isEmpty ? 0 : widget.index.clamp(0, letters.length - 1);
    _currentLetter = letters.isEmpty ? const Letter(char: '', words: ['']) : letters[_entryIndex];
  ...
  void _buildOptions() {
    final pool = List<Letter>.from(letters);
    pool.removeWhere((entry) => entry.char == _currentLetter.char);

    final chosen = <Letter>[_currentLetter];
  ...
  void _playInstruction() {
    final displayLetter = _isUppercase
        ? _currentLetter.char.toUpperCase()
        : _currentLetter.char.toLowerCase();
    AudioService.instance.speak('Selecciona la letra $displayLetter');
  }

  Future<void> _onOptionPressed(Letter selected) async {
    if (_locked) return;

    if (selected.char == _currentLetter.char) {
      ...
      try {
        ...
        await Future.delayed(const Duration(milliseconds: 600));
        final hasNext = _entryIndex < letters.length - 1;
        if (hasNext) {
          ...
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (_) => SelectionLetterScreen(
                index: nextIndex,
