...
class CompleteRandomLettersScreen extends StatefulWidget {
  ...
  const CompleteRandomLettersScreen({
    Key? key,
    required this.index,
    this.word = '',
  ...
  @override
  State<CompleteRandomLettersScreen> createState() => _CompleteRandomLettersScreenState();
}

class _CompleteRandomLettersScreenState extends State<CompleteRandomLettersScreen> {
  ...
  void initState() {
    ...
    _missingIndexes = _pickMissingIndexes(_word);
    _slots = List<String?>.generate(_word.length, (i) => _missingIndexes.contains(i) ? null : _word[i]);
    _setupPool();
  }

  List<int> _pickMissingIndexes(String word) {
    final hideCount = word.length < 3 ? word.length : 3;
    final Set<int> indexes = {};
    while (indexes.length < hideCount) {
      indexes.add(_random.nextInt(word.length));
    }
  ...
  Future<void> _onCorrectComplete() async {
    ...
    if (hasNext) {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (_) => CompleteRandomLettersScreen(index: nextIndex, initialIsUppercase: _isUppercase)),
      );
    } else {
            ...
            child: Row(
              ...
              children: [
                ...
                IconButton(
                  ...
                  color: const Color.fromARGB(255, 55, 35, 28),
                  onPressed: () {
                    AudioService.instance.speak('Completa la palabra con las letras que faltan');
                  },
                ),
              ],
            ),
                      ...
                      if (hasPrev)
                        IconButton(
                          ...
                          onPressed: () => Navigator.pushReplacement(
                            context,
                            MaterialPageRoute(
                              builder: (_) => CompleteRandomLettersScreen(index: prevIndex, initialIsUppercase: _isUppercase),
                            ),
                          ),
                      ...
                      if (hasNext)
                        IconButton(
                          ...
                          onPressed: () => Navigator.pushReplacement(
                            context,
                            MaterialPageRoute(
                              builder: (_) => CompleteRandomLettersScreen(index: nextIndex, initialIsUppercase: _isUppercase),
                            ),
                          ),
